...
ASSUNTO_EMAIL = "Bem-vindo(a) ao Complexo Hospitalar dos Estivadores"
...
<body style="font-family:Calibri,Arial,sans-serif;font-size:14px;color:#222;">
  ...
  <div style="background-color:#c7fbfd;padding:18px 22px;max-width:640px;">
    <p><b>Bem-vindo(a) ao Complexo Hospitalar dos Estivadores</b></p>

    <p>É com grande satisfação que recebemos {nome_primeiro} no Complexo
    Hospitalar dos Estivadores. Estamos entusiasmados em tê-lo(a) em nossa
    equipe e desejamos muito sucesso em sua nova jornada.</p>

    <p>Para que possa começar a utilizar nossas ferramentas internas e se
    integrar às nossas plataformas de trabalho, suas credenciais de acesso
    estão detalhadas no arquivo PDF anexo.<br>
    O documento está protegido. Para acessá-lo, utilize seu
    <b>CPF (apenas os números, sem pontos ou traços)</b> como senha.<br>
    Este documento contém seu SSHD (Serviço de Segurança Humana e Digital),
    que será utilizado como credencial de acesso às estações de trabalho e
    aplicações do sistema MV.</p>

    <p><b>Informamos que sua conta se encontra, inicialmente, inativada.</b><br>
    A ativação só ocorrerá após o reset de senha, que deve ser realizado
    acessando o link:</p>

    <p><a href="https://egov.santos.sp.gov.br/websshd/">https://egov.santos.sp.gov.br/websshd/</a></p>

    <p>As credenciais de acesso às demais aplicações deverão ser obtidas
    seguindo os passos disponíveis na intranet (atalho disponível na área
    de trabalho), acessando o menu <b>Unidades &gt; CHE &gt; Credenciais de
    acesso</b>.</p>
...
COR_FUNDO = "#f4fbfb"
COR_PRIMARIA = "#0f8f9e"
COR_PRIMARIA_ESCURA = "#0b6e79"
COR_TEXTO_SUAVE = "#6b7b7d"
...
class App(tk.Tk):
    ...
    def _aplica_tema(self):
        ...
        style.configure("TLabel", background=COR_FUNDO, foreground="#20302f")
        style.configure("TLabelframe", background=COR_FUNDO, foreground=COR_PRIMARIA_ESCURA,
                         font=("Segoe UI", 10, "bold"))
        style.configure("TLabelframe.Label", background=COR_FUNDO, foreground=COR_PRIMARIA_ESCURA,
                         font=("Segoe UI", 10, "bold"))
        style.configure("TCheckbutton", background=COR_FUNDO)
        style.configure("TButton", padding=6)
        style.configure("Accent.TButton", background=COR_PRIMARIA, foreground="white",
                         font=("Segoe UI", 9, "bold"))
        style.map("Accent.TButton", background=[("active", COR_PRIMARIA_ESCURA)])
        style.configure("Rodape.TLabel", background=COR_FUNDO, foreground=COR_TEXTO_SUAVE,
                         font=("Segoe UI", 8))
    ...
    def _monta_cabecalho(self):
        ...
        try:
            self._logo1_img = tk.PhotoImage(data=base64.b64decode(LOGO1_B64))
            self._logo2_img = tk.PhotoImage(data=base64.b64decode(LOGO2_B64))
            tk.Label(frm, image=self._logo1_img, bg="white").pack(side="left", padx=(14, 6), pady=8)
            tk.Label(frm, image=self._logo2_img, bg="white").pack(side="left", padx=6, pady=8)
        except Exception:
            pass

        titulo_frm = tk.Frame(frm, bg="white")
        titulo_frm.pack(side="left", padx=16)
        tk.Label(
            titulo_frm, text="Envio de Credenciais - CHE",
            bg="white", fg=COR_PRIMARIA_ESCURA, font=("Segoe UI", 13, "bold"),
        ).pack(anchor="w")
        tk.Label(
            titulo_frm, text="Desenvolvido por Kaue Dias",
            bg="white", fg=COR_TEXTO_SUAVE, font=("Segoe UI", 8, "italic"),
        ).pack(anchor="w")

        tk.Frame(self, bg=COR_PRIMARIA, height=3).pack(fill="x")
    ...
    def _monta_interface(self):
        ...
        self.btn_enviar = ttk.Button(frm_botoes, text="2) Enviar de verdade", command=self.enviar, style="Accent.TButton")
        ...
        self.log_box = scrolledtext.ScrolledText(self, height=18, borderwidth=1, relief="solid")
        self.log_box.pack(fill="both", expand=True, padx=8, pady=(8, 8))
